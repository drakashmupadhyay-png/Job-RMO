...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tableViewContainer = document.getElementById('table-view-container'), calendarViewContainer = document.getElementById('calendar-view-container');
    ...
    const statusOptions = ['Identified', 'Preparing Application', 'Applied', 'Interview Offered', 'Offer Received', 'Unsuccessful']; 
    ...
    function renderTable() {
        ...
        const tableContainerEl = tableViewContainer.querySelector('.table-container');
        let emptyState = tableViewContainer.querySelector('.empty-state-container');
        ...
        if (processedJobs.length === 0) {
            ...
            tableViewContainer.appendChild(emptyState);

            if (searchBar.value || currentFilters.state !== 'all' || currentFilters.status !== 'all') {
                emptyState.innerHTML = `<i class="fa-solid fa-magnifying-glass"></i><h3>No Matches Found</h3><p>No applications match your current filters.</p>`;
            } else {
                emptyState.innerHTML = `<i class="fa-solid fa-folder-open"></i><h3>No Applications Yet</h3><p>Click "Add Application" to get started!</p>`;
            }
            return;
        }
        ...
        processedJobs.forEach(job => {
            ...
            row.innerHTML = `
                ...
                <td><strong>${job.jobTitle || 'N/A'}</strong><br><small>${job.hospital || 'N/A'}</small></td>
                <td>${job.state || 'N/A'}</td>
    ...
    function populateApplicationDetailPage(job) {
        ...
        renderDocuments(stagedDocuments);
    ...
    function attachMainAppEventListeners() {
        ...
        searchBar.addEventListener('input', () => { currentFilters.search = searchBar.value; renderTable(); });
        ...
        applicationDetailPage.addEventListener('click', handleDocumentButtons);
    ...
    function handleFilterChange(e) { const key = e.target.id.replace('-filter', '').replace('role-level', 'roleLevel').replace('sort-by', 'sortBy'); currentFilters[key] = e.target.value; renderTable(); }
    function handleClearFilters() { filterControls.querySelectorAll('select, input').forEach(el => { if (el.id === 'sort-by-filter') el.value = 'default'; else if(el.type === 'search') el.value = ''; else el.value = 'all'; }); currentFilters = { state: 'all', type: 'all', status: 'all', search: '', sortBy: 'default', roleLevel: 'all' }; renderTable(); }
    ...
    function enterSelectMode() { isSelectMode = true; selectionActionBar.classList.remove('hidden'); renderTable(); }
    function exitSelectMode() { isSelectMode = false; selectedJobIds.clear(); updateSelectionCount(); selectionActionBar.classList.add('hidden'); renderTable(); }
    ...
    // MISC HANDLERS
    function handleDocumentButtons(e) { if (e.target.closest('.add-doc-btn')) { const docType = e.target.closest('.add-doc-btn').dataset.docType; const input = document.getElementById(`add-${docType}-doc-input`); if (input.value.trim()) { stagedDocuments.push({ id: Date.now(), type: docType, name: input.value.trim() }); renderDocuments(stagedDocuments); input.value = ''; } } else if (e.target.closest('.remove-doc-btn')) { const docId = parseInt(e.target.closest('.remove-doc-btn').dataset.docId); stagedDocuments = stagedDocuments.filter(doc => doc.id !== docId); renderDocuments(stagedDocuments); } }
